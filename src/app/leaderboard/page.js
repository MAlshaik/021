...
const SocialIcon = ({ platform, username }) => {
  ...
  return (
    <a href={`https://${platform}.com/${username}`} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-400">
      {icons[platform]}
    </a>
  );
};
...
export default function LeaderboardPage() {
  // Sort the leaderboard data based on launchCycles in descending order
  const sortedLeaderboardData = [...leaderboardData].sort((a, b) => b.launchCycles - a.launchCycles);
            ...
            {sortedLeaderboardData.map((user, index) => (
              <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-2 md:gap-4 px-4 py-3 text-sm hover:bg-white/25 transition-colors">
                ...
                <div className="flex space-x-2">
                  <SocialIcon platform="twitter" username={user.twitter} />
                  <SocialIcon platform="linkedin" username={user.linkedin} />
                  <SocialIcon platform="instagram" username={user.instagram} />
                </div>
              </div>
            ))}
          </div>
